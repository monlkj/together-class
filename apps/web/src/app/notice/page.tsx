...
async function myMemoryTranslate(text: string, to: string): Promise<string> {
  try {
    ...
    const res = await fetch(
      `https://api.mymemory.translated.net/get?q=${encodeURIComponent(text)}&langpair=ko|${toCode}`
    );
...
export default function NoticePage() {
  ...
  // Google Cloud Vision 전송 전 이미지 리사이즈 + base64 변환
  ...
  const resizeForOcr = (file: File): Promise<Blob> =>
    new Promise((resolve) => {
      const img = new Image();
  ...
  const handleAnalyze = async () => {
    ...
    await Promise.all(
      selectedLangs.map(async (lang) => {
        translations[lang] = await myMemoryTranslate(noticeText, lang);
      })
    );
  ...
  const shareUrl = 'https://dahamkke.app/notice/share?id=20261015';
  const copyLink = () => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={styles.card}>
        ...
        <button
          ...
          style={{
            ...styles.analyzeBtn,
            opacity: (loading || ocrLoading || !noticeText.trim() || selectedLangs.length === 0) ? 0.5 : 1,
          }}
        >
          ...
          <div style={styles.summaryCard}>
            ...
            <div style={styles.summaryGrid}>
              <div style={styles.summaryItem}>
                <span style={styles.summaryLabel}>제목</span>
                <span style={styles.summaryValue}>{result.summary.title}</span>
              </div>
              <div style={styles.summaryItem}>
                <span style={styles.summaryLabel}>일시</span>
                <span style={styles.summaryValue}>{result.summary.date}</span>
              </div>
              <div style={styles.summaryItem}>
                <span style={styles.summaryLabel}>준비물</span>
                <span style={styles.summaryValue}>{result.summary.items}</span>
              </div>
              <div style={styles.summaryItem}>
                <span style={styles.summaryLabel}>제출 기한</span>
                <span style={styles.summaryValue}>{result.summary.due}</span>
              </div>
            </div>

            <div style={styles.shareRow}>
              <button onClick={() => setShowShare(!showShare)} style={styles.shareBtn}>
                📱 학부모 공유 QR / 링크 생성
              </button>
              {showShare && (
                <div style={styles.shareBox}>
                  <div style={styles.qrPlaceholder}>
                    <div style={styles.qrGrid}>
                      {Array.from({ length: 25 }).map((_, i) => (
                        <div key={i} style={{ ...styles.qrCell, backgroundColor: Math.random() > 0.5 ? '#1F2937' : '#FFFFFF' }} />
                      ))}
                    </div>
                  </div>
                  ...
                  <p style={styles.shareHint}>학부모님께 QR을 보내거나 링크를 공유하면 모국어로 열람할 수 있습니다.</p>
                </div>
              )}
            </div>
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  progressBar: {
    height: '8px', backgroundColor: '#D1FAE5', borderRadius: '4px', overflow: 'hidden',
  },
  progressFill: {
  ...
  qrPlaceholder: {
    ...
    padding: '8px', backgroundColor: '#F9FAFB',
  ...
  shareUrlRow: { display: 'flex', alignItems: 'center', gap: '8px', width: '100%' },
  shareUrl: {
  ...
  shareHint: { margin: 0, fontSize: '11px', color: '#9CA3AF', textAlign: 'center' },
